...
class CMDManager extends events_1.EventEmitter {
    ...
    async EventHandler(client, collection) {
        const table = new AsciiTable3("Events")
            ...
            .setAlignCenter(3);
        const Files = await (0, functions_1.FileLoader)(this._EventsLocation, this._TypeScript);
        collection.clear();
        Files.forEach((file) => {
    ...
    async CommandHandler(client, collection, execCollection) {
        ...
        let commandsArray = [];
        let developerArray = [];
        const Files = await (0, functions_1.FileLoader)(this._CommandDirectory, this._TypeScript);
        Files.forEach((file) => {
            let FiEx = ".js";
